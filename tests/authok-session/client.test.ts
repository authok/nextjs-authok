...
import { getConfig, ConfigParameters } from '../../src/authok-session';
...
const getClient = async (params: ConfigParameters = {}): Promise<Client> => {
  const { default: clientFactory } = await import('../../src/authok-session/client');
...
describe('clientFactory', function () {
  ...
  it('should send the correct default headers', async function () {
    ...
    expect(headerProps).toContain('authok-client');

    const decodedTelemetry = JSON.parse(Buffer.from(headers['authok-client'] as string, 'base64').toString('ascii'));

    expect(decodedTelemetry.name).toEqual('nextjs-authok');
    ...
    expect(headers['user-agent']).toEqual(`nextjs-authok/${pkg.version}`);
  });
  ...
  it('should disable telemetry', async function () {
    ...
    expect(headerProps).not.toContain('authok-client');
  });
  ...
  it('should create custom logout for authok', async function () {
    nock('https://test.cn.authok.cn')
      .get('/.well-known/openid-configuration')
      .reply(200, { ...wellKnown, issuer: 'https://test.cn.authok.cn/', end_session_endpoint: undefined });
    nock('https://test.cn.authok.cn').get('/.well-known/jwks.json').reply(200, jwks);
