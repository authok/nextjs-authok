...
const createHandlers = (params: ConfigParameters): Handlers => {
  ...
  const getClient = clientFactory(config, { name: 'nextjs-authok', version });
...
export const setup = async (
  ...
  nock('https://test.cn.authok.cn', { allowUnmocked: true }).persist().get('/.well-known/jwks.json').reply(200, jwks);
