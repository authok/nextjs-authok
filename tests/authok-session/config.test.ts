import { Config, ConfigParameters, getConfig, AuthorizationParameters, DeepPartial } from '../../src/authok-session';
...
describe('Config', () => {
  ...
  it('should set idpLogout to true when authokLogout is true', () => {
    const config = getConfig({
      ...defaultConfig,
      authokLogout: true
    });
  ...
  it('authokLogout and idpLogout should default to false', () => {
    const config = getConfig(defaultConfig);
    expect(config).toMatchObject({
      authokLogout: false,
      idpLogout: false
  ...
  it('should not set authokLogout to true when idpLogout is true', () => {
    const config = getConfig({
      ...defaultConfig,
      idpLogout: true
    });
