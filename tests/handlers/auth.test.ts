...
import { get } from '../authok-session/fixtures/helpers';
import { initAuthok, OnError, Session } from '../../src';
...
import * as baseLoginHandler from '../../src/authok-session/handlers/login';
import * as baseLogoutHandler from '../../src/authok-session/handlers/logout';
import * as baseCallbackHandler from '../../src/authok-session/handlers/callback';
...
describe('auth handler', () => {
  ...
  test('return 500 for unexpected error', async () => {
    ...
    global.handleAuth = initAuthok(withoutApi).handleAuth;
...
describe('custom error handler', () => {
  ...
  test('finish response if custom error does not', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    global.handleAuth = initAuthok(withoutApi).handleAuth.bind(null, { onError });
    await expect(
...
describe('custom handlers', () => {
  ...
  test('accept custom login handler', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    global.handleAuth = initAuthok(withoutApi).handleAuth.bind(null, { login });
  ...
  test('accept custom logout handler', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    global.handleAuth = initAuthok(withoutApi).handleAuth.bind(null, { logout });
  ...
  test('accept custom callback handler', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    global.handleAuth = initAuthok(withoutApi).handleAuth.bind(null, { callback });
  ...
  test('accept custom profile handler', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    global.handleAuth = initAuthok(withoutApi).handleAuth.bind(null, { profile });
  ...
  test('accept custom arbitrary handler', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    global.handleAuth = initAuthok(withoutApi).handleAuth.bind(null, { signup });
...
describe('custom options', () => {
  ...
  test('accept custom login options', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    const { handleLogin, handleAuth } = initAuthok(withoutApi);
  ...
  test('accept custom logout options', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    const { handleLogout, handleAuth } = initAuthok(withoutApi);
    global.handleAuth = handleAuth.bind(null, {
  ...
  test('accept custom callback options', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    const { handleCallback, handleAuth } = initAuthok(withoutApi);
    global.handleAuth = handleAuth.bind(null, {
  ...
  test('accept custom profile options', async () => {
    ...
    const baseUrl = await setup(withoutApi);
    const { handleProfile, handleAuth } = initAuthok(withoutApi);
    global.handleAuth = handleAuth.bind(null, {
