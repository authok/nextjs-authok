...
import { makeIdToken } from '../authok-session/fixtures/cert';
import { defaultConfig, get, post, toSignedCookieJar } from '../authok-session/fixtures/helpers';
...
import { signing as deriveKey } from '../../src/authok-session/utils/hkdf';
...
describe('callback handler', () => {
  ...
  test('should validate the issuer', async () => {
    ...
    ).rejects.toThrow('unexpected iss value, expected https://acme.authok.local/, got: other-issuer');
  });
  ...
  test('should create the session without OIDC claims with api config', async () => {
    ...
    expect(session).toStrictEqual({
      ...
      idToken: await makeIdToken({ iss: 'https://acme.authok.local/' }),
      user: {
        nickname: '__test_nickname__',
        sub: '__test_sub__'
      }
