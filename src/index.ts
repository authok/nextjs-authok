 ...
 *
 * This is created for you when you use the named exports, or you can create your own using {@link InitAuthok}.
 *
 * See {@link ConfigParameters} for more info.
 *
 * @category Server
 */
export interface AuthokServer {
  /**
   * Session getter.
   */
  ...
  /**
   * Login handler which will redirect the user to Authok.
   */
  ...
  /**
   * Logout handler which will clear the local session and the Authok session.
   */
 ...
 * @category Server
 */
export type InitAuthok = (params?: ConfigParameters) => AuthokServer;

let instance: AuthokServer & { sessionCache: SessionCache };

// For using managed instance with named exports.
function getInstance(): AuthokServer & { sessionCache: SessionCache } {
...
export const _initAuth = (params?: ConfigParameters): AuthokServer & { sessionCache: SessionCache } => {
  const { baseConfig, nextConfig } = getConfig(params);

  // Init base layer (with base config)
  const getClient = clientFactory(baseConfig, { name: 'nextjs-authok', version });
